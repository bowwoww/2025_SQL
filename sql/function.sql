--custom function
CREATE or alter FUNCTION fnGetDeadline(@dateValue DATETIME)
RETURNS VARCHAR(20)
AS
BEGIN
    ...
    RETURN CONVERT(VARCHAR(20), DATEADD(DAY, 10, @dateValue), 120);
END
GO

declare @t VARCHAR(20)
set @t = dbo.fnGetDeadline('2023-10-01');
print 'Deadline is: ' + @t;
go

drop FUNCTION fnGetDeadline;
GO

use [北風]
GO
-- custom function
CREATE or alter FUNCTION fnGetEmployeeName(@employeeID INT)
...
BEGIN
    ...
    SELECT @employeeName = 員工.姓名 FROM 員工 WHERE 員工.員工編號 = @employeeID;
...
set @name = dbo.fnGetEmployeeName(@id);
print 'Employee Name is: ' + @name;

drop FUNCTION fnGetEmployeeName;
go


create or alter function fnGetCustomerByKeyword(@keyword NVARCHAR(50))
RETURNS TABLE
AS
RETURN
(
    SELECT *
    FROM dbo.客戶 as C
    WHERE C.公司名稱 LIKE '%' + @keyword + '%' or C.客戶編號 like '%' + @keyword + '%'
);
GO

declare @keyword NVARCHAR(50) = '公司';
select * from fnGetCustomerByKeyword(@keyword);
GO

-- custom function 取得一個新的流水編號其格式為日期+該日第幾張訂單
-- 例如202310010001表示2023年10月1日的第一張訂單(8碼日期+4碼流水號)
CREATE OR ALTER FUNCTION fnGetNewOrderNumber(@orderDate DATE)
RETURNS NVARCHAR(12)
AS
BEGIN
    DECLARE @newOrderNumber NVARCHAR(12);
    select @newOrderNumber = count(*) + 1
    from dbo.訂貨主檔 as OrderTable
    where CONVERT(DATE, OrderTable.訂單日期) = @orderDate;
    IF @newOrderNumber IS NULL
        SET @newOrderNumber = 1;
    set @newOrderNumber = FORMAT(@orderDate, 'yyyyMMdd') + RIGHT('0000' + CAST(@newOrderNumber AS NVARCHAR(4)), 4);
    RETURN @newOrderNumber;
END
GO

declare @orderDate DATE = '1998-05-06';
declare @newOrderNumber NVARCHAR(12);
set @newOrderNumber = dbo.fnGetNewOrderNumber(@orderDate);
print 'New Order Number for is: ' + @newOrderNumber;

select * from dbo.訂貨主檔 where 訂單日期 = '1998-05-06';
GO
...
-------------------------------------
--建立Product資料內容
insert into [Products] (ProductID, ProductName, Price, [Description], Picture,CataID) values
...
insert into [Products] values(dbo.getProductID('A2'),'test',50,'testtest','A2001.jpg','A2')
GO

create or ALTER function getProductID(@CateID nchar(2))
	returns nchar(5)
as
begin
...
--測試
insert into [Products] values(dbo.getProductID('A4'),'test',50,'testtest',dbo.getProductID('A4')+'.jpg',getdate(),'A4')
GO
...
BEGIN
    --檢查會員帳號是否存在
    IF EXISTS (SELECT 1 FROM [Member] WHERE [Account] = @Account)
    BEGIN
        RAISERROR('會員帳號已存在', 16, 1)
        RETURN
    END

    --新增會員資料
    INSERT INTO [Member] (MemberID, Name, Gender, MemberPoint, Account, Password,CreatedDate)
    VALUES (@MemberID, @Name, @Gender, 0, @Account, @Password, GETDATE());
END
GO
--測試新增會員
EXEC AddMember @MemberID = 'M0001', @Name = '測試用戶', @Gender = 1, @Account = 'testuser', @Password = 'password123'
